...
{
    public sealed class GetTaskQueryHandler(IApplicationDbContext context)
        : IRequestHandler<GetTaskQuery, GetTaskResponse>
    {
        public async Task<GetTaskResponse> Handle(GetTaskQuery request, CancellationToken cancellationToken)
        {
            var task = await context.Tasks
              ...
              .Select(x => new GetTaskResponse(x.Id, x.ProjectId, x.Project.Name, x.Title, x.Description, x.Status, x.Priority,
