...
{
    public sealed class GetTaskQueryHandler(IApplicationDbContext context) : IRequestHandler<GetTaskQuery, GetTaskResponse>
    {
        public async Task<GetTaskResponse> Handle(GetTaskQuery request, CancellationToken cancellationToken)
        {
            var task = await context.Tasks.AsNoTracking()
                ...
                x.UpdatedAtUtc, x.Dependencies.Count, x.Labels.Count, x.ChecklistItems.Count, x.Attachments.Count,
                x.Comments.Count)).FirstOrDefaultAsync(cancellationToken);
